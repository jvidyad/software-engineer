...
def reverse_hash_any_length(hash_val):
    length=1
    letters = "acdegilmnoprstuw"
    while True:
        low = Hash(letters[0] * length)
        high = Hash(letters[-1] * length)

        if ((low <= hash_val) and (high >= hash_val)):
            break

        else:
            length +=1

    return reverse_hash(length, hash_val)
    
def main():
    hash_val = 930846109532517
    
    print reverse_hash_any_length(hash_val)
